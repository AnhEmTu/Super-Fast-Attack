...
local Tabs = {
    ...
    Raid = Window:AddTab({ Title = "tập kích", Icon = "Kiếms" }),
...
elseif SelectMaterial == "Kim Loại" then
if First_Sea then
MMon = "Brute"
MPos = CFrame.new(-1145,15,4350)
SP = "Default"
...
SP = "Default"
elseif SelectMaterial == "Súngpowder" then
...
for i,v in pairs(game:GetService("Workspace").NPCs:GetChildren()) do
    pcall(function()
        if LADESP then 
            if v.Name == "Legendary Kiếm Dealer" then
                if not v:FindFirstChild('NameEsp') then
                    local bill = Instance.new('BillboardGui',v)
                    bill.Name = 'NameEsp'
                    bill.ExtentsOffset = Vector3.new(0, 1, 0)
...
local plr = game.Players.LocalPlayer
    ...
    function AttackNoCoolDown()
        if not AutoFarmMasDevilFruit or AutoFarmMasSúng then
...
local listfastattack = {'Tấn công bình thường','Tấn công nhanh','Tấn công siêu nhanh'}
    ...
    local DropdownSelectWeapon = Tabs.Main:AddDropdown("DropdownSelectWeapon", {
        ...
        Description = "",
        Values = {'Cận Chiến','Kiếm','Trái Ác Quỷ'},
        Multi = false,
        Default = 1,
    })
...
local boss = Tabs.Main:AddSection("Farm Boss")
    ...
    if First_Sea then
        ...
        MaterialList = {
          "Kim loại phế liệu","Da","Demonic Wisp","Cacao kết hợp","Vảy rồng","Bột bột","Đuôi cá","Ngà nhỏ"
        }
        end
...
if Second_Sea then
    local ToggleKiếmLengend = Tabs.Main:AddToggle("ToggleKiếmLengend", {Title = "Mua Kiếm Huyền Thoại",Description = "",Default = false })
    ToggleKiếmLengend:OnChanged(function(Value)
        _G.BuyLengendKiếm = Value
    end)
        Options.ToggleKiếmLengend:SetValue(false)

        spawn(function()
            while wait(.1) do
                pcall(function()
                    if _G.BuyLengendKiếm or Triple_A then
                        local args = {
                            [1] = "LegendaryKiếmDealer",
                            [2] = "2"
                        }
                        -- Triple_A
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
...
local ToggleCận Chiến = Tabs.Stats:AddToggle("ToggleCận Chiến", {Title = "Nâng Cận Chiến",Description = "", Default = false })
ToggleCận Chiến:OnChanged(function(Value)
    _G.Auto_Stats_Cận Chiến = Value
    end)
Options.ToggleCận Chiến:SetValue(false)
...
local ToggleKiếm = Tabs.Stats:AddToggle("ToggleKiếm", {Title = "Nâng Kiếm",Description = "", Default = false })
ToggleKiếm:OnChanged(function(Value)
    _G.Auto_Stats_Kiếm = Value
    end)
Options.ToggleKiếm:SetValue(false)



local ToggleSúng = Tabs.Stats:AddToggle("ToggleSúng", {Title = "Nâng Súng", Description = "",Default = false })
ToggleSúng:OnChanged(function(Value)
    _G.Auto_Stats_Súng = Value
    end)
Options.ToggleSúng:SetValue(false)
...
spawn(function()
    while wait() do
        if _G.Auto_Stats_Cận Chiến then
            local args = {
                [1] = "AddPoint",
                [2] = "Cận Chiến",
                [3] = 3
...
if First_Sea then
 IslandList = {
"Cối xay gió", "Hàng hải", "Thị trấn trung tâm", "Rừng", "Làng cướp biển", "Sa mạc", "Đảo Tuyết", "MarineFord", "Đấu trường La Mã", "Đảo trời 1", "Đảo bầu trời 2", "Đảo trời 3", "Nhà tù", "Làng magma", "Đảo dưới nước", "Thành phố đài phun nước", "Phòng Chân", "Đảo Mob",
}
...
local Chips = {"Lửa","Băng","Quake","Ánh Sáng","Bóng Tối","Tơ","Rumble","Magma","Phật","Cát","Phượng Hoàng","Bột"}
...
Tabs.Misc:AddButton({
	...
	Callback = function()
		...
		for k, v in pairs(allitem) do
			...
			for k, v in pairs(v) do
				if Items[v.ItemName.Text] and Items[v.ItemName.Text].Mastery then
					...
					v.Parent = cac
				elseif v.ItemLine2.Text == "Trái Ác Quỷ" then
					v.Parent = cac2
				end
			end
			cac.AutomaticSize = 2
		...
		local Cận ChiếnG = Instance.new("Frame", Left)
		Cận ChiếnG.BackgroundTransparency = 1
		Cận ChiếnG.Size = UDim2.new(1, 0, 0, 0)
		Cận ChiếnG.LayoutOrder = table.find(RaityLevel, k)
		Cận ChiếnG.AutomaticSize = 2
		Cận ChiếnG.LayoutOrder = 100
		local tvk = Instance.new("UIGridLayout", Cận ChiếnG)
		tvk.CellPadding = UDim2.new(.005, 0, .005, 0)
		tvk.CellSize = UDim2.new(0, 70, 0, 70)
		tvk.FillDirectionMaxCells = 100
		tvk.FillDirection = Enum.FillDirection.Horizontal
		...
		for k, v in pairs(cac) do
			if ListHuhu[v] and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Buy" .. v, true) == 1 then
				local huhu = Instance.new("ImageLabel", Cận ChiếnG)
